...
import (
	...
	_ "github.com/ajwerner/livedit/giopkgs"
...
var (
	gomacroLogoImg = loadImage("gophernotes-gopher.png")
	gomacroLogoOp  = imageOpOrEmpty(gomacroLogoImg)
	...
	return layout.Flex{
		...
		layout.Flexed(1, func(gtx C) D {
			return layout.Flex{
				...
				layout.Flexed(.5, func(gtx C) D {
					return layout.Center.Layout(gtx, func(gtx C) D {
						return squareLogo(gtx, gomacroLogoImg, &gomacroLogoOp)
					})
				}),
			)
		}),
		...
		case system.FrameEvent:
			...
			if containsChange(editor.Events()) || first {
				...
				func() {
					...
					gomacro.Comp.CompGlobals.Universe.CachePackage(types.NewPackage("live", "live"))
					_, err := gomacro.EvalReader(strings.NewReader(editor.Text()))
					if err != nil {
						fmt.Println(err)
						return
					}
					result := gomacro.ValueOf("Layout")
					var (
						ok        bool
